...
namespace ToMoToStudy.Controllers
{
    public class AjaxController : Controller
    {
        ...
        [HttpPost]
        public JsonResult XoaChiTietCTHoc(int id)
        {
            ApiResult result = new ApiResult();
            using (ToMoToDBEntities db = new ToMoToDBEntities())
            {
                var cthoc = db.CT_HOC.Where(x => x.IdCTHoc == id).FirstOrDefault();
                var lop = db.LopHocs.Any(x => x.IdCTHoc == id);
                if (cthoc is null)
                {
                    result.Message = "Vui lòng thử lại";
                    return Json(result);
                ...
                if (lop is true)
                {
                    result.Message = "Đang có lớp học sử dụng chương trình này";
                    return Json(result);
                }
                else
                {
        ...
        public JsonResult ThemCTBaiCu(CHITIET_CT_HOC data)
        {
            ...
            using (ToMoToDBEntities db = new ToMoToDBEntities())
            {
                ...
                cthoc.NgayBatDau = data.NgayBatDau;
                if (data.NgayBatDau == null)
                    cthoc.NgayBatDau = DateTime.Now;
                if (data.IdChiTiet == 0)
                {
        ...
        [HttpPost]
        public JsonResult ThemTuLuanCu( int idcauhoituluan,int idtuluan)
        {
            ApiResult result = new ApiResult();
            using (ToMoToDBEntities db = new ToMoToDBEntities())
            {
                ...
                var tuluancauhoi=db.TuLuan_CauHoi.Where(x=>x.IdCauHoiTuLuan== idcauhoituluan).FirstOrDefault();
                if (tuluancauhoi is null || tuluan is null)
                {
                    result.Message = "Vui lòng thử lại";
                    return Json(result);
                }
                ...
                tuluan.TuLuan_CauHoi.Add(tuluancauhoi);
                try
                {
                    db.SaveChanges();
                    result.Success = true;
        ...
        [HttpPost]
        public JsonResult GetTuLuanCauHoi(int idcauhoituluan)
        {
            using (ToMoToDBEntities db = new ToMoToDBEntities())
            {
                var tuluancauhoi = db.TuLuan_CauHoi.Where(x => x.IdCauHoiTuLuan == idcauhoituluan).FirstOrDefault();
                ...
                return new JsonResult()
                {
                    Data = new {cauhoituluan = tuluancauhoi.CauHoiTuLuan },
                    JsonRequestBehavior = JsonRequestBehavior.AllowGet
                };
            }
        ...
        [HttpPost]
        public JsonResult NopBai(SinhVien_TL_DapAn[] lstDapAn)
        {
            ApiResult result = new ApiResult();
            if (lstDapAn is null || lstDapAn.Length < 1)
            {
                result.Message = "Vui lòng điền đầy đủ thông tin bên trên";
                return Json(result.Data);
            }
            ...
            using (ToMoToDBEntities db = new ToMoToDBEntities())
            {
                ...
                var sv = db.SinhViens.Where(x => x.IdNguoiDung == nd.IdNguoiDung).FirstOrDefault();
                foreach (var dapan in lstDapAn)
                {
                    var daDB = db.SinhVien_TL_DapAn.Where(x => x.IdCauHoiTuLuan == dapan.IdCauHoiTuLuan && x.SinhVien.IdNguoiDung == nd.IdNguoiDung).FirstOrDefault();
                    if (daDB != null)
                    {
                        daDB.DapAn = dapan.DapAn;
                        continue;
                    }

                    dapan.IdSinhVien = sv.IdSinhVien;
                    db.SinhVien_TL_DapAn.Add(dapan);
                }
                try
                {
                    db.SaveChanges();
        ...
        [HttpPost]
        public JsonResult PostFileHocVien(FileGiangVien data, HttpPostedFileBase file)
        {
            ApiResult result = new ApiResult();
            if (data is null)
            {
            ...
            using (ToMoToDBEntities db = new ToMoToDBEntities())
            {
                FileGiangVien filegiangvien = new FileGiangVien();
                // chỉnh sửa tk
                if (data.IdFileThongBao > 0) filegiangvien = db.FileGiangViens.Where(x => x.IdFileThongBao == data.IdFileThongBao).FirstOrDefault();
                if (filegiangvien is null)
                {
                    result.Message = "Vui lòng thử lại";
                    return Json(result);
                }
                filegiangvien.Link = data.Link;
                filegiangvien.NoiDung = data.NoiDung;
                filegiangvien.IdBaiHoc = data.IdBaiHoc;
                if (data.IdFileThongBao == 0)
                {
                    filegiangvien.NgayTao = DateTime.Now;
                    db.FileGiangViens.Add(filegiangvien);
                }
                try
                {
                    if (file != null)
                    {
                        ...
                        file.SaveAs(path);
                        filegiangvien.NoiDung = fileName;
                        db.SaveChanges();
        ...
        [HttpPost]
        public JsonResult TrinhTrangBaiHoc(BaiHoc data)
        {
            ApiResult result = new ApiResult();
            if (data is null)
            {
            ...
            using (ToMoToDBEntities db = new ToMoToDBEntities())
            {
                ...
                // chỉnh sửa tk
                if (data.IdBaiHoc > 0) bh = db.BaiHocs.Where(x => x.IdBaiHoc == data.IdBaiHoc).FirstOrDefault();
                if (bh is null)
                {
                    result.Message = "Vui lòng thử lại";
                    return Json(result);
                ...
                bh.ChoPhepNopBai = data.ChoPhepNopBai;
        ...
        [HttpPost]
        public JsonResult ThemTracNghiemCu(int idcauhoi, int idtracnghiem)
        {
            ApiResult result = new ApiResult();
            using (ToMoToDBEntities db = new ToMoToDBEntities())
            {
                ...
                var tracnghiemcauhoi = db.TracNghiem_CauHoi.Where(x => x.IdCauHoi == idcauhoi).FirstOrDefault();
                if (tracnghiem is null || tracnghiemcauhoi is null)
                {
                    result.Message = "Vui lòng thử lại";
                    return Json(result);
                }
                ...
                //tuluancauhoi.IdTuLuan = data.IdTuLuan;
                tracnghiem.TracNghiem_CauHoi.Add(tracnghiemcauhoi);
                try
                {
                    db.SaveChanges();
                    result.Success = true;
        ...
        [HttpPost]
        public JsonResult XoaTracNghiemCauHoi(int idtracnghiem,int idcauhoi)
        {
            ApiResult result = new ApiResult();
            using (ToMoToDBEntities db = new ToMoToDBEntities())
            {
                var tracnghiem = db.TracNghiems.Where(x => x.IdTracNghiem == idtracnghiem).FirstOrDefault();
                var tracnghiemcauhoi = db.TracNghiem_CauHoi.Where(x => x.IdCauHoi==idcauhoi).FirstOrDefault();
                if (tracnghiemcauhoi is null || tracnghiem is null)
                {
                    result.Message = "Vui lòng thử lại";
                    return Json(result);
                }
                try
                {
                    tracnghiem.TracNghiem_CauHoi.Remove(tracnghiemcauhoi);
                    db.SaveChanges();
                    result.Success = true;
                }
                catch (Exception ex)
        ...
        [HttpPost]
        public JsonResult XoaBinhLuan(BinhLuan data)
        {
            ApiResult result = new ApiResult();
            if (data is null)
            {
            ...
            using (ToMoToDBEntities db = new ToMoToDBEntities())
            {
                ...
                if (data.IdBinhLuan > 0)
                    binhluan = db.BinhLuans.Where(x => x.IdBinhLuan == data.IdBinhLuan).FirstOrDefault();
        ...
        [HttpPost]
        public JsonResult ChamDiemTuLuan(SinhVien_TL_DapAn data)
        {
            ApiResult result = new ApiResult();
            if (data is null)
            {
            ...
            using (ToMoToDBEntities db = new ToMoToDBEntities())
            {
                SinhVien_TL_DapAn svdapan = new SinhVien_TL_DapAn();
                svdapan = db.SinhVien_TL_DapAn.Where(x => x.IdSinhVien == data.IdSinhVien && x.IdCauHoiTuLuan==data.IdCauHoiTuLuan).FirstOrDefault();
                if (svdapan is null)
                {
                    result.Message = "Vui lòng thử lại";
                    return Json(result);
                }
                svdapan.DiemTuLuan = data.DiemTuLuan;
                try
                {
                    db.SaveChanges();
                    result.Success = true;
        ...
        public JsonResult DSTN(int idcauhoi)
        {
            using (ToMoToDBEntities db = new ToMoToDBEntities())
            {
                var tracnghiemcauhoi = db.TracNghiem_CauHoi.Where(x => x.IdCauHoi == idcauhoi).FirstOrDefault();
        ...
        public JsonResult DSTL(int idcauhoi)
        {
            using (ToMoToDBEntities db = new ToMoToDBEntities())
            {
                var tuluancauhoi = db.TuLuan_CauHoi.Where(x => x.IdCauHoiTuLuan == idcauhoi).FirstOrDefault();
                if (tuluancauhoi is null) return null;
                return new JsonResult()
                {
                    Data = new { noidungcauhoi = tuluancauhoi.CauHoiTuLuan },
                    JsonRequestBehavior = JsonRequestBehavior.AllowGet
                };
            }
        ...
        [HttpPost]
        public JsonResult SuaCTHoc(CT_HOC data)
        {
            ApiResult result = new ApiResult();
            if (data is null)
            {
